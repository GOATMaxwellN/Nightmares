...
public class GunScript : MonoBehaviour
{
    ...
    private void Shoot()
    {
        ...
        if (Physics.Raycast(gunray, out RaycastHit hitInfo, 150f, enemylayer))
        {
            ...
            if (hitInfo.collider.gameObject.TryGetComponent(out EnemyScriptSpawn enemyhit2))
            {
                enemyhit2.takedamage(damage);
                print("HIT");
            }

            if (hitInfo.collider.gameObject.TryGetComponent(out EnemyScript_OLD enemyhit3))
            {
                enemyhit3.takedamage(damage);
                print("HIT");
            }
        }
    }
